...
# Если запуск через pyinstaller, то используем sys._MEIPASS, иначе — текущий каталог
...
try:
    ...
    raise AttributeError("Модель не содержит атрибут feature_names_. Убедитесь, что это обученная модель с поддержкой feature_names.")

missing_features = [f for f in required_features if f not in df.columns]
if missing_features:
    raise ValueError(f"Отсутствуют следующие признаки в данных: {missing_features}")
